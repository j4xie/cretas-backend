...
public class EquipmentController {
    ...
    public ApiResponse<EquipmentDTO> updateEquipment(
            ...
            @PathVariable @NotNull Integer equipmentId,
            @Valid @RequestBody CreateEquipmentRequest request) {
    ...
    public ApiResponse<List<EquipmentDTO>> importEquipment(
            @Parameter(description = "工厂ID", required = true)
            @PathVariable @NotBlank String factoryId,
            @Parameter(description = "访问令牌", required = true)
            @RequestHeader("Authorization") String authorization,
            @Valid @RequestBody List<CreateEquipmentRequest> requests) {

        String token = TokenUtils.extractToken(authorization);
        Integer userId = mobileService.getUserFromToken(token).getId();

        log.info("批量导入设备: factoryId={}, count={}", factoryId, requests.size());
        List<EquipmentDTO> equipment = equipmentService.importEquipment(factoryId, requests, userId);
        return ApiResponse.success(String.format("成功导入%d个设备", equipment.size()), equipment);
    }
    ...
    public byte[] exportEquipmentList(
            @Parameter(description = "工厂ID", required = true)
            @PathVariable @NotBlank String factoryId) {

        log.info("导出设备列表: factoryId={}", factoryId);
        return equipmentService.exportEquipmentList(factoryId);
    }
