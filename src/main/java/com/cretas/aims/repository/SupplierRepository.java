...
public interface SupplierRepository extends JpaRepository<Supplier, Integer> {
     ...
     * 根据ID和工厂ID查找供应商
     */
    Optional<Supplier> findByIdAndFactoryId(Integer id, String factoryId);
    ...
    @Query("SELECT s FROM Supplier s WHERE s.factoryId = :factoryId " +
           "AND (s.name LIKE %:keyword% OR s.supplierCode LIKE %:keyword%)")
     ...
     * 检查供应商名称是否存在（排除指定ID）
     */
    boolean existsByFactoryIdAndNameAndIdNot(String factoryId, String name, Integer id);
    ...
    boolean hasRelatedMaterialBatches(@Param("supplierId") Integer supplierId);
