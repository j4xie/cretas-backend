...
public interface ConversionRepository extends JpaRepository<MaterialProductConversion, Integer> {
     ...
     * 根据原材料类型查找转换率
     */
    List<MaterialProductConversion> findByFactoryIdAndMaterialTypeId(String factoryId, Integer materialTypeId);

    /**
     * 根据产品类型查找转换率
     */
    List<MaterialProductConversion> findByFactoryIdAndProductTypeId(String factoryId, Integer productTypeId);

    /**
     * 查找特定原材料和产品的转换率
     */
    Optional<MaterialProductConversion> findByFactoryIdAndMaterialTypeIdAndProductTypeId(
            String factoryId, Integer materialTypeId, Integer productTypeId);
    ...
    @Query("SELECT DISTINCT c.productTypeId FROM MaterialProductConversion c " +
           ...
           "AND c.isActive = true")
    List<Integer> findProductTypesByMaterialType(@Param("factoryId") String factoryId,
