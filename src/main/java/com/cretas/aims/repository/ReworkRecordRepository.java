...
public interface ReworkRecordRepository extends JpaRepository<ReworkRecord, Long> {
     ...
     * 根据生产批次ID查询返工记录
     */
    List<ReworkRecord> findByProductionBatchId(Long productionBatchId);
     ...
     * 查询返工成功率统计
     */
    @Query("SELECT AVG(CASE WHEN r.reworkQuantity > 0 " +
           "THEN (r.successQuantity / r.reworkQuantity) * 100 " +
           "ELSE 0 END) " +
           "FROM ReworkRecord r WHERE r.factoryId = :factoryId AND r.status = 'COMPLETED'")
